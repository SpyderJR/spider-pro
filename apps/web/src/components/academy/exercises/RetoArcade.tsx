...
type Data = Extract<Exercise, { kind: "retoArcade" }>;

export function RetoArcade({ data, onSolved }: { data: Data; onSolved: () => void }) {
  ...
  return (
    <div className={`bg-void-soft rounded-xl p-4 border ${met ? "border-neon-green/50" : "border-void-border"}`}>
      <div className="text-[10px] font-mono font-bold tracking-widest text-neon-gold mb-2">RETO EN EL ARCADE</div>
      ...
      <div className="flex items-center justify-between">
        <Link to="/app/arcade" className="text-xs font-mono text-neon-blue hover:underline">
          → Jugar {data.gameLabel}
        </Link>
